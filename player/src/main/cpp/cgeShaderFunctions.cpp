...
#include "include/cgeGLFunctions.h"
#include "include/cgeShaderFunctions.h"
...
namespace CGE
{
	...
	bool ShaderObject::init(GLenum shaderType)
	{
		...
		CGE_LOG_INFO("cgeShaderFunctions init %d",m_shaderID);
		return m_shaderID != 0;
	}
	...
	bool ShaderObject::loadShaderSourceFromString(const char* shaderString)
	{
        ...
        CGE_LOG_INFO("cgeShaderFunctions loadShaderSourceFromString %d,%0x",compiled,m_shaderType);
...
//					glUniform1f(uniformID, float(sz.width) / sz.height / (*iter)->uniformValue[0].valuef);
	...
	{
	    CGE_LOG_INFO("cgeShaderFunctions linkWithShaderObject %d",m_programID);
		if(m_programID != 0)
		{
		...
		CGE_LOG_INFO("cgeShaderFunctions Attach Shaders in useProgram，%d，%d",vertObj.shaderID(),fragObj.shaderID());
